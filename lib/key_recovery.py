...
from lib.constants import CACHE_OFFSET, FIRST_PLAINTEXT_BITS, PLAINTEXT_BYTE_NUM, SETS_NUM
...
def recover_msb_key_from_cache_misses_lines(cache_misses_lines: list[int], byte_index: int) -> str:
    ...
    # Take into account that the first table starts at table index 16*3 + 2. We have both a table number offset of 2 and a initial table set offset of 2. 
    base_table_set = (((byte_index % 4) + CACHE_OFFSET) % 4 * 16) + CACHE_OFFSET # T0:34-49, T1:50-01, T2:02-17, T3:18-33 
    candidates = []
    ...
    for plaintext_int in range(PLAINTEXT_BYTE_NUM):
        ...
        print(f"P_hi=0x{plaintext_int:X}: Base Table Set={base_table_set} Cache Miss set={cache_miss_set}, Set index inside the table={table_set_index}")
